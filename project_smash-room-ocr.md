...
- **状態A**: OBS 展開、対象ソースは畳み。スペーサーは **対象ソースカードの下** に置き余白を吸収（カード同士の間には伸びない）。監視ボタン＋`label_status` は非表示。セットアップ時にステータス行が見えないため、切断時は `messagebox`、接続エラー時は（状態Aなら）`messagebox` で補足。
...
- **`build_exe.bat`**: PyInstaller で単一 `SmashArenaIDScanner.exe` を生成（`--hidden-import` に `winsdk` 関連および `winsdk.windows.applicationmodel.datatransfer` 等を指定）。
...
- **無駄・注意点**:
  ...
  - **CLI で `PyInstaller ... main.py` を実行すると `SmashArenaIDScanner.spec` が上書き生成される**。カスタム `excludes` / `optimize` / UPX 等を spec で管理するなら、**編集済み spec を `pyinstaller SmashArenaIDScanner.spec` でビルドする運用**の方が安全。
...
| `test_pipeline.py` | ローカル画像 / OBS 連携の統合検証スクリプト。 |
| `test_boot.py` | 短時間の起動スモーク。 |
| `_treq.py` | 開発用: 接続カード setup/run の要請高さをコンソールに出すワンオフ（本番起動経路では未使用）。 |
...
- 回帰確認用に `test_sample_suite.py` を追加し、`test_screen_<正解>_<誤読>.png` 規約の固定サンプルで一括検証できるようにした（今回サンプルで 7/7 正解）。
...
- そのため **コード内のマジックナンバーだけに固定しない**。実装段階では次を満たすこと:
  ...
  - 既定値は仕様書に「現在の推奨」と明記し、変更履歴は `development_log.md` 等で追えるようにする。
...
- **現在の仕様の正**: 本ファイル（`project_smash-room-ocr.md`）。
- **`development_log.md`**: 経緯・検討メモ。過去の「次ステップ」と現状が食い違う場合は **本仕様を優先**。
