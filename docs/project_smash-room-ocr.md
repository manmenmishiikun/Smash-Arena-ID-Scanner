...
| `chrome-extension/content.js` | pathname ゲート・`button.matching_post_btn` 存在時のみ入力反映。受信部屋IDは Python の `ROOM_ID_PATTERN` と同じ正規表現で検証。`autoClickMatchingPostBtn` が true のとき入力後に同ボタンを `click()`（オプションは `storage` キャッシュ + `onChanged` で同期） |
...
| `detection_poll_fast_sec` | 探索中のフレーム間スリープ秒（既定 `1.0`、0.05〜60）。 |
